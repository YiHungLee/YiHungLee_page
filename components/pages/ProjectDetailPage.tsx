...
const ProjectDetailPage: React.FC = () => {
  ...
  if (!project) {
    return (
      <div className="min-h-screen bg-warmCream-50 dark:bg-darkMode-bg flex items-center justify-center transition-colors duration-500">
        <div className="text-center space-y-6">
          <h1 className="font-display text-4xl md:text-5xl font-bold text-charcoal-900 dark:text-darkMode-text">
            找不到此作品
          </h1>
          <Link
            to="/projects"
            className="inline-block font-body text-sm tracking-wide uppercase
                       text-ochre-500 dark:text-darkMode-ochre editorial-underline">
            返回作品集
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-warmCream-50 dark:bg-darkMode-bg transition-colors duration-500">
      ...
      <article className="relative pt-40 pb-20 md:pt-48 md:pb-32
                          bg-warmCream-100 dark:bg-darkMode-bgElevated transition-colors duration-500 subtle-texture">
        <div className="max-w-4xl mx-auto px-6 md:px-12">
          ...
          <div className="mb-12 opacity-0 animate-fade-in">
            <Link
              ...
              className="font-body text-xs tracking-widest uppercase
                         text-charcoal-600 dark:text-darkMode-textMuted editorial-underline">
          ...
          <header className="space-y-8 md:space-y-12">
            ...
            <div className="flex flex-wrap items-center gap-4 opacity-0 animate-fade-in-up stagger-1">
              <p className="font-body text-xs tracking-widest uppercase text-charcoal-600 dark:text-darkMode-textMuted">
                {categoryLabels[category as ProjectCategory]}
              </p>
              <div className="h-4 w-px bg-border-light dark:bg-darkMode-border"></div>
              <p className="font-body text-sm text-charcoal-500 dark:text-darkMode-textFaint">{project.year}</p>
              {project.award && (
                <>
                  <div className="h-4 w-px bg-border-light dark:bg-darkMode-border"></div>
                  <div className="inline-block px-3 py-1 border border-fine
                                  border-ochre-500 dark:border-darkMode-ochre font-body text-xs tracking-wide
            ...
            <div className="opacity-0 animate-fade-in-up stagger-2">
              <h1 className="font-display text-4xl md:text-5xl lg:text-6xl font-bold
                             text-charcoal-900 dark:text-darkMode-text tracking-tight leading-tight optical-align">
                {project.title}
              </h1>
            ...
            {project.award && (
              <div className="opacity-0 animate-fade-in-up stagger-3">
                <p className="font-accent italic text-lg text-ochre-600 dark:text-darkMode-ochre">
                  {project.award}
                </p>
              </div>
            )}
            ...
            {project.venue && (
              <div className="opacity-0 animate-fade-in-up stagger-4">
                <p className="font-body text-base text-charcoal-600 dark:text-darkMode-textMuted">
                  {project.venue}
                </p>
              </div>
            )}
        ...
        <div className="max-w-4xl mx-auto px-6 md:px-12">
          ...
          {(project.tracks && project.tracks.length > 0) || project.audioUrl ? (
            <div className="mb-12 opacity-0 animate-fade-in-up stagger-8">
              <h2 className="font-display text-2xl font-bold text-charcoal-900 dark:text-darkMode-text mb-6">
            ...
            <div className="mb-12 opacity-0 animate-fade-in-up stagger-7">
              ...
              <div className="flex flex-wrap gap-3">
                {project.techStack.map((tech) => (
                  <span
                    key={tech}
                    className="font-body text-sm tracking-wide text-charcoal-700 dark:text-darkMode-textMuted
                               px-4 py-2 border border-fine border-border-light dark:border-darkMode-border
                               hover:border-ochre-500 dark:hover:border-darkMode-ochre transition-colors duration-300">
                    {tech}
                  </span>
                ))}
              </div>
                ...
                {project.githubUrl && (
                  <a
                    ...
                    className="font-body text-sm tracking-wide uppercase
                               text-charcoal-700 dark:text-darkMode-textMuted editorial-underline
            ...
            <div className="opacity-0 animate-fade-in-up stagger-10">
              <div className="flex flex-wrap gap-3">
                {project.tags.map((tag) => (
                  <span
                    key={tag}
                    className="font-body text-xs tracking-wide text-charcoal-600 dark:text-darkMode-textMuted
                               px-3 py-1 border border-fine border-border-light dark:border-darkMode-border">
                    {tag}
                  </span>
                ))}
              </div>
